...
#+begin_src emacs-lisp
  (custom-set-faces
   ...
   ;; no background: a selection stays the only grey band, so the two never
   ;; look alike. :extend draws the rule past end-of-line, orange reads on both
   ;; the dark and the light theme
   '(bm-face ((t (:extend t :underline (:color "#da8548" :style line)))))
