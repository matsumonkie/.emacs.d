...
#+begin_src emacs-lisp
  ...
  (defun my-toggle-theme ()
    ...
    (let ((dark 'doom-one)
      ...
      (if (member dark custom-enabled-themes)
          (progn
            (disable-theme dark)
            (load-theme light t))
        (when (member light custom-enabled-themes)
          (disable-theme light))
        (load-theme dark t))))
...
#+begin_src emacs-lisp
  ...
  (global-set-key (kbd "M-r") 'bm-next)
  (global-set-key (kbd "M-s") 'bm-previous)

  (setq bm-highlight-style 'bm-highlight-only-fringe)

  ;; TTY has no fringe: draw the marker in the left margin instead
  (defun my/bm-margin-marker ()
    (propertize " " 'display `((margin left-margin)
                               ,(propertize ">" 'face (bm-get-highlight-face-fringde)))))

  (define-advice bm-get-fringe-marker (:around (orig) tty-margin)
    (if (display-graphic-p) (funcall orig) (my/bm-margin-marker)))

  ;; reserve the margin column
  (defun my/bm-reserve-margin ()
    (unless (display-graphic-p)
      (setq left-margin-width (max left-margin-width 1))
      (when (get-buffer-window) (set-window-buffer nil (current-buffer)))))
  (add-hook 'find-file-hook #'my/bm-reserve-margin)
...
#+begin_src emacs-lisp
  ...
  (install-package 'ob-mermaid)
  (setq ob-mermaid-cli-path "/home/iori/.npm-global/bin/mmdc")
...
#+begin_src emacs-lisp
  ...
  (setq org-startup-with-inline-images t) ;; always show images. toggle with: `C-c C-x C-v`

  ;; Add mermaid and other languages to org-babel
  (org-babel-do-load-languages
      'org-babel-load-languages
      '((mermaid . t)
        (scheme . t)))

  ;; Open file with specific software (when using `C-c C-o`)
  (setq org-file-apps
        '((auto-mode . emacs)
          ("\\.png\\'" . "xdg-open %s")
          ("\\.svg\\'" . "xdg-open %s")))
...
#+begin_src emacs-lisp
      ...
      (eval-after-load "haskell-mode"
        '(progn
           ...
           (define-key haskell-mode-map (kbd "M-.") 'xref-find-definitions)
           ))
    ...
    ;;  (define-key haskell-mode-map (kbd "M-r") 'haskell-interactive-mode-history-next)
      ...
      (add-hook 'lsp-mode-hook #'my/haskell-mode-setup)
...
#+begin_src emacs-lisp
  ...
  ;; always launch 2 tabs, first one should be magit
  ...
  ;; paragraph are 90 characters long
...
#+begin_src emacs-lisp
  ...
  ;; run every 15min
  ;; uncomment in winter season when we don't really need the light theme
